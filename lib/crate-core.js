...
CrateCore.prototype.remoteInsert = function(ei){
    ...
    if (ei._i._s){
		this.emit('remoteCaretMoved', ei._i._s[0], {
			start: index,
			end: index
		});
    }else{
		// anti-entropy insert message
		this.emit('remoteCaretMoved', ei._i.t.s, {
			start: index,
			end: index
		});
    }
...
CrateCore.prototype.remoteRemove = function(id){
    ...
    if (id._i._s){
		this.emit('remoteCaretMoved', id._s[0], {
			start: index - 1,
			end: index - 1
		});
    }else{
		// anti-entropy insert message
		this.emit('remoteCaretMoved', id.t.s, {
			start: index - 1,
			end: index - 1
		});
};
...
CrateCore.prototype.remoteCaretMoved = function(param){
    this.emit('remoteCaretMoved', param.guid, param.range);
};
